...
class ColorSpaces:
    ...
    def ReadImage(self):
        image_matrix = cv.imread(self.image)
        cv.imshow(winname='image_window',mat=image_matrix)
        cv.waitKey(0)
        pass

    def ColortoGray(self):
        gray_image = cv.cvtColor(src=self.image,code=cv.COLOR_BGR2GRAY)
        cv.imshow(winname='gray_image',mat=gray_image)
        cv.waitKey(0)
...
image_file = 'C:/Users/Arjun Janamatti/Documents/image_classification/nude_sexy_safe_v1_x320/testing/safe/0A556B72-059A-4BFE-AA28-80728C324749.jpg'
image = ColorSpaces(image_file)
image.ReadImage()