...
sentence_list_1 = ["Hi", "How are you how", "Is anyone there?", "Hello", "Good day", "Whats up"]

words_corpus = []
sentence_list = []

for sentences in sentence_list_1:
    tokenized_sentences = (word_tokenize(text=sentences))
    sentence_list.append(tokenized_sentences)
    words_corpus.extend(tokenized_sentences)
...
element_count_term_frequency = {}
for index,sentence in enumerate(sentence_list):
    for words in sentence:
        print(words)
        if words in element_count_term_frequency:
            # incerementing the count by 1
            element_count_term_frequency[words+'_'+str(index)] += 1
        else:
            # setting the count to 1
            element_count_term_frequency[words+'_'+str(index)] = 1
    # print(sentence)
print(element_count_term_frequency)